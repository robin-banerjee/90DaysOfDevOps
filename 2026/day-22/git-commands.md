...
| `git clone <url>` | clone an existing repo | `git clone https://github.com/robin-banerjee/90DaysOfDevOps.git` |
| `git add file` | add untrack files | `git add demo.txt` |
| `git commit -m` | make a commit | `git commit -m "added demo.txt"` |
| `git reset file` | resets staged changes | `git reset demo.txt` |
| `git rm --cached <file>` | to untrack a file that was already committed | `git rm --cached demo.txt` |
